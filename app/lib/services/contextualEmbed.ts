 ...
 * с новыми префиксами.
 *
 * Извлечение контекста — keyword-based, не LLM. На наших нишах достаточно,
 ...
 *   - vet ПЕРЕД dental: «ветклиника со стоматологией» — это vet, не dental.
 *   - renovation ПЕРЕД home-services: «ремонт квартир с заменой электрики»
 *     не должен ловиться ключом «электрик».
 *   - digital-agency БЕЗ ключей «разработка сайтов»/«лендинг»/«сделать сайт» —
 *     иначе словим обычные запросы к самому NIT («сделай лендинг кофейни»).
 */
const NICHE_KEYWORDS: Array<[string, string[]]> = [
  ...
  ["vet", ["ветеринар", "ветклиник", "ветлечебниц", "питомц"]],
  ...
  ["renovation", ["ремонт квартир", "ремонт под ключ", "отделочн", "штукатур", "стяжк"]],
  ["home-services", ["сантехник", "электрик", "засор", "вызов мастера", "муж на час"]],
  ["digital-agency", ["веб-студи", "веб студи", "диджитал", "digital-агентств", "digital agency"]],
  ["moving", ["переезд", "грузоперевозк", "грузчик", "перевозка мебели"]],
  ["hotel", ["отель", "гостиниц", "гостевой дом", "хостел", "апарт-отел"]],
  // niche "translation" есть в сидах (medical-translation), но раньше отсутствовал
  // здесь → query-сторона НЕ получала [translation]-префикс, который сид получил
  // на индексации (train/serve mismatch, занижавший cosine именно для этого сида).
  ["translation", ["перевод документ", "переводчик", "бюро перевод", "нотариальн перевод", "медицинск перевод", "локализац"]],
];
