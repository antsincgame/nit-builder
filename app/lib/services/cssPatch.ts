...
export function rulesToCss(rules: CssRule[]): string {
  // Жёсткая гарантия: CSS-патч не может вырваться из <style>. В легальных
  // селекторах и значениях угловых скобок не бывает, поэтому вырезаем '<'/'>'
  // безусловно — даже если модель по запросу юзера вернёт "</style><script>".
  const noAngles = (s: string): string => s.replace(/[<>]/g, "");
  return rules
    .map((rule) => {
      const props = Object.entries(rule.properties)
        .map(([key, value]) => {
          const v = noAngles(value).replace(/\s*!important\s*$/i, "").trim();
          return `  ${noAngles(key)}: ${v} !important;`;
        })
        .join("\n");
      return `${noAngles(rule.selector)} {\n${props}\n}`;
    })
    .join("\n\n");
}
