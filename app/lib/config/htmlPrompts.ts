...
11. Если в user-мессадже есть блок РАЗМЕТКА КОЛЛЕКЦИЙ — для каждой коллекции создай в её секции РОВНО ОДИН элемент-образец: контейнер списка с data-collection="<id>", внутри ровно одна карточка с атрибутом data-item, поля карточки с data-field="<field_id>". Не дублируй карточку руками — PHP-цикл размножит её сам. data-field для image — на самом <img>. Значения образца заполни реалистично: они станут первой записью таблицы. id коллекций и полей — дословно из блока.
...
- Атрибуты data-edit="…", data-edit-type="…", data-edit-label="…", а также data-collection, data-item, data-field сохраняются как есть, не удалять и не переименовывать (используются post-processor'ом для PHP-админки).
...
Адаптируй шаблон под план и дизайн-токены${copyHint ? ", вставь готовый копирайт дословно" : ""}${zonesHint ? ", расставь три data-edit атрибута на каждую размеченную зону" : ""}${collectionsHint ? ", создай по одному элементу-образцу на каждую коллекцию (data-collection/data-item/data-field)" : ""}. Верни готовый HTML.`;
}
...
- Если в user-мессадже есть блоки РАЗМЕТКА РЕДАКТИРУЕМЫХ ЗОН или РАЗМЕТКА КОЛЛЕКЦИЙ — выполни их правила полностью: три data-edit атрибута на каждую зону, один элемент-образец с data-collection/data-item/data-field на каждую коллекцию.
...
7. Сохрани атрибуты data-edit, data-edit-type, data-edit-label, data-collection, data-item, data-field — не удалять и не переименовывать.
