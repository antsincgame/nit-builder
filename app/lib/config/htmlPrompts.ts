...
- services — 3-6 РЕАЛЬНЫХ услуг/направлений бизнеса для секции услуг (services/programs/subjects/practices/courses/classes). Это то, ЧТО бизнес делает («Наращивание ресниц», «Чистка лица», «Замена масла»), а НЕ выгоды (выгоды — это key_benefits, «почему мы»). Каждая: title (название 1-4 слова) + description (1 предложение конкретики). Заполняй для сервисных ниш (бьюти, клиника, барбершоп, фитнес, репетитор, юристы, автосервис, школа, йога). Если у бизнеса нет дискретных услуг (кафе=меню, SaaS=features) — ПРОПУСКАЙ. ВАЖНО: перечисли ВСЕ основные услуги (обычно 4-6), а не 1-2 — иначе в шаблоне останутся дефолтные карточки.
