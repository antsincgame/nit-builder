...
10. Если в user-мессадже есть блок РАЗМЕТКА РЕДАКТИРУЕМЫХ ЗОН — добавь атрибут data-edit="<id>" на узлы, соответствующие зонам. Ровно один узел на каждый id. Правила:
    - type=text → узел inline-уровня с одной строкой текста: h1/h2/h3/span/a или короткий <p>. Атрибут на самом узле, не на родителе.
    ...
    - type=image → элемент <img>. Атрибут на самом <img>.
    - id используешь дословно из блока (snake_case). Не придумывай свои id, не дублируй один id на разных узлах.
...
- Атрибуты data-edit="…" сохраняются как есть, не удалять и не переименовывать (используются post-processor'ом для PHP-админки).
...
Адаптируй шаблон под план и дизайн-токены${copyHint ? ", вставь готовый копирайт дословно" : ""}${zonesHint ? ", расставь data-edit атрибуты на размеченные зоны" : ""}. Верни готовый HTML.`;
}
...
7. Сохрани атрибуты data-edit="…" — не удалять и не переименовывать.
