...
describe("extractSectionHtml", () => {
  it("снимает markdown-ограждения и берёт <section>", () => {
    expect(extractSectionHtml("```html\n<section><h2>X</h2></section>\n```")).toBe(
      "<section><h2>X</h2></section>",
    );
  });
  it("вытаскивает section из окружающего текста", () => {
    expect(extractSectionHtml("Вот секция: <section>A</section> готово")).toBe("<section>A</section>");
  });
  it("без тегов — возвращает очищенный текст как есть", () => {
    expect(extractSectionHtml("  просто текст  ")).toBe("просто текст");
  });
});

describe("SectionFlow — редьюсер посекционной генерации", () => {
  const ds = buildSectionDesignSystem(basePlan);
  const goodHero =
    "<section id='h'><h1>Кофе</h1><p>Лучший кофе города каждый день и для каждого гостя всегда.</p></section>";
  const goodContact =
    "<section id='c'><h2>Контакты</h2><p>Звоните и заходите к нам каждый день с утра до позднего вечера.</p></section>";

  it("initSectionFlow берёт очередь из plan.sections", () => {
    const flow = initSectionFlow(basePlan, ds);
    expect(flow.queue).toEqual(["hero", "features", "contact"]);
    expect(flow.index).toBe(0);
    expect(flow.blocks).toEqual([]);
  });

  it("startSectionFlow даёт generate первой секции с её брифом", () => {
    const step = startSectionFlow(initSectionFlow(basePlan, ds));
    expect(step.kind).toBe("generate");
    if (step.kind === "generate") {
      expect(step.sectionName).toBe("hero");
      expect(step.prompt).toContain("Лучший кофе в городе");
      expect(step.system).toContain("ОДНУ секцию");
    }
  });

  it("happy path: валидные секции принимаются, на последней — done со сборкой", () => {
    const plan = { ...basePlan, sections: ["hero", "contact"] } as Plan;
    const flow = initSectionFlow(plan, ds);
    const r1 = advanceSectionFlow(flow, goodHero, { maxRetries: 1 });
    expect(r1.step.kind).toBe("generate");
    expect(r1.state.index).toBe(1);
    expect(r1.state.blocks).toHaveLength(1);

    const r2 = advanceSectionFlow(r1.state, goodContact, { maxRetries: 1 });
    expect(r2.step.kind).toBe("done");
    if (r2.step.kind === "done") {
      expect(r2.step.html).toContain("id='h'");
      expect(r2.step.html).toContain("id='c'");
      expect(r2.step.html).toContain("<!DOCTYPE html>");
      expect(r2.step.html).toContain("nit-section-base");
    }
  });

  it("невалидная секция ретраится (та же секция, index не двигается)", () => {
    const r = advanceSectionFlow(initSectionFlow(basePlan, ds), "   ", { maxRetries: 1 });
    expect(r.step.kind).toBe("generate");
    if (r.step.kind === "generate") expect(r.step.sectionName).toBe("hero");
    expect(r.state.index).toBe(0);
    expect(r.state.retries).toBe(1);
  });

  it("после лимита ретраев принимает что есть, чтобы не зациклиться", () => {
    const flow = { ...initSectionFlow(basePlan, ds), retries: 1 };
    const r = advanceSectionFlow(flow, "<section></section>", { maxRetries: 1 });
    expect(r.state.index).toBe(1);
    expect(r.state.retries).toBe(0);
    expect(r.state.blocks).toHaveLength(1);
  });

  it("обрыв по длине (truncated) не принимается при наличии ретраев", () => {
    const r = advanceSectionFlow(initSectionFlow(basePlan, ds), goodHero, {
      maxRetries: 1,
      truncated: true,
    });
    expect(r.step.kind).toBe("generate");
  ...
  it("не мутирует исходное состояние", () => {
    const flow = initSectionFlow(basePlan, ds);
    advanceSectionFlow(flow, goodHero, { maxRetries: 1 });
    expect(flow.index).toBe(0);
    expect(flow.blocks).toEqual([]);
  });
});
